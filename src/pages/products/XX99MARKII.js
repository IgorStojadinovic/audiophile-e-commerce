...
const XX99MARKII = () => {
    ...
    const handleSubtract = () => {
        if(quantity !== 0) {
           dispatch(updateQuantity({type:'remove-markII'}))
        }
    }

    const hanbleAdd = () => {
        dispatch(updateQuantity({type:'add-markII'}))
    }

    const addItems = (quantity) =>{
        if (quantity !== 0) {
            const item = {name:'xx99mkII',total:2999 * quantity,defaultPrice:2999,quantity:quantity}
